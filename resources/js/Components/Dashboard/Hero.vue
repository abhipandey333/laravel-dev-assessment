...
const emit = defineEmits(["update:title", "update:location"]);

const localTitle = computed({
  get: () => props.title,
  set: (newVal) => emit("update:title", newVal),
});

const localLocation = computed({
  get: () => props.location,
  set: (newVal) => emit("update:location", newVal),
});
</script>
      ...
      <form @submit.prevent="onSearch">
        <div>
          ...
          >
            ...
            <button
              class="bg-brand px-6 text-sm font-medium py-2 rounded-full text-white mr-3"
              type="submit"
            >
              Find jobs
            </button>
